...
const getAllAthletesDetailed = utils.dreamCatcher(async (req, res) => {
  const { user } = req;
  if (
    user.role !== utils.ROLES.ADMIN &&
    user.role === utils.ROLES.COACH &&
    !user.club
  ) {
  ...
  const athletes =
    user.role === utils.ROLES.ADMIN
      ? utils.mapDbObjectToResponse(
          await athletesQueries.getDetailedAllAthletes(req.db)
        )
      : utils.mapDbObjectToResponse(
          await athletesQueries.getDetailedAllAthletesByClub(
            req.db,
            user.club.id
          )
        );
...
const getSingleOrAllAthletes = utils.dreamCatcher(async (req, res) => {
  if (req.params.id) {
    const athlete = await athletesQueries.findAthleteById(
      req.db,
      req.params.id
    );
    if (!(athlete.length > 0)) {
  ...
  const athletes = utils.mapDbObjectToResponse(
    await athletesQueries.getAllAthletes(req.db)
  );
  return res.json({ athletes });
});
...
const createAthlete = utils.dreamCatcher(async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      error:
        "Something went wrong, reverting previous steps, check logs for details.",
    });
  }
});
...
const updateAthlete = utils.dreamCatcher(async (req, res) => {
  ...
  const oldAthlete = await athletesQueries.getAthleteCleanBasicInfoById(
    req.db,
    req.params.id
  );
  ...
  const savedAthlete = await athletesQueries.updateAthlete(
    req.db,
    newAthleteInfo
  );
...
const addMatchForSingleAthlete = utils.dreamCatcher(async (req, res) => {
  ...
  if (
    ...
    const athlete = (
      await athletesQueries.findAthleteById(req.db, athleteId)
    )[0];
    bout.athleteId = athleteId;
    bout.athleteName = athlete.name;
    bout.athleteClubShortHand = athlete.club_shorthand;
  }
  const validBout = await schema.boutSchema.validateAsync(
    bout,
    schema.defaultValidationOptions
  );
...
const recalculateAthleteAchievement = utils.dreamCatcher(async (req, res) => {
  ...
  return res.status(200).json({
    message: `Successfully triggered recalculation for id: ${athleteId}`,
  });
});
