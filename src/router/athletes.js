...
athleteRouter.get("/:_id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching athelte, check logs for details" });
  }
});
...
athleteRouter.post("/", async (req, res, next) => {
  ...
  try {
    ...
    if (error.name === "ValidationError") {
      ...
      return res
        .status(500)
        .json({ message: "Error saving athlete, check logs for details" });
    }
  }
});
...
athleteRouter.put("/:_id", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error updating athlete, check logs for details" });
  }
});
...
athleteRouter.post("/:_id/bouts", async (req, res, next) => {
  ...
  try {
    ...
    if (error.name === "ValidationError") {
      ...
      return res
        .status(500)
        .json({ message: "Error saving bout, check logs for details" });
    }
  }
});
